...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('apiKeys') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "Running [ $commandName ] should remove all apiKey variables" {
            ...
            $DattoRMMModuleApiSecretKey | Should -BeNullOrEmpty
        }



        It "If the [ DattoRMMModuleApiKey ] is already empty a warning should be thrown" {
            Add-DattoRMMAPIKey -ApiKey '12345' -ApiSecretKey '12345'
            Remove-Variable -Name "DattoRMMModuleApiKey" -Scope Global -Force

            Remove-DattoRMMAPIKey -WarningAction SilentlyContinue -WarningVariable apiKeyWarning
            $apiKeyWarning | Should -Be "The Datto API key is not set. Nothing to remove"
        }
