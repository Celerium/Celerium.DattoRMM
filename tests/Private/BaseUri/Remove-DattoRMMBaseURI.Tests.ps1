<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-DattoRMMBaseURI.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-DattoRMMBaseURI.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('baseUri') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "The baseUri variable should not longer exist" {
            ...
            $DattoRMMModuleBaseUri | Should -BeNullOrEmpty
        }

        It "If the baseUri is already gone a warning should be thrown" {
            Add-DattoRMMBaseURI -DataCenter Zinfandel
            Remove-DattoRMMBaseURI
            Remove-DattoRMMBaseURI -WarningAction SilentlyContinue -WarningVariable baseUriWarning
